...
async function tokenGeneration() {
  //add this one
 // if (TOKEN != "a6d5716d-8627-4ae8-bd34-a32674f208cd") {
  if (TOKEN != " ") {
    token = TOKEN;
    console.log(token);
    //add this
     
  //} else if (AUTH_URL != "63689514ea334ad8cd28a0f1bf1cf95384df4572273106802148fcbaab4af182") {
  } else if (AUTH_URL != " ") {
    token = await window
...
async function startMeeting(token, meetingId, name) {
  ...
  meeting.on("meeting-joined", () => {
    meeting.pubSub.subscribe("CHAT", (data) => {
      ...
      const chatTemplate = `
          <div style="margin-block-end: 10px; ${meeting.localParticipant.id == senderId && "text-align : right"
        }">
